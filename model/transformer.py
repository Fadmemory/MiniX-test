...
class FeaturesTransformer(nn.Module):
    ...
    def mixed_y_embedding(self, y:dict, y_type:torch.Tensor, eval_pos:int):
        ...
        # Determine output dtype: use the dtype from the encoder outputs if available, otherwise use model dtype
        output_dtype = self.dtype if self.dtype is not None else torch.float32
        if cls_y_emb is not None:
            output_dtype = cls_y_emb.dtype
        elif reg_y_emb is not None:
            output_dtype = reg_y_emb.dtype

        out = torch.empty(len(y_flat), emb_size, dtype=output_dtype, device=y_flat.device)
        if cls_y_emb is not None:
